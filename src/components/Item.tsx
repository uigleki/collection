...
function ReviewContent({ point }: { point: ReviewPoint }) {
  return (
    <>
      <span className="text-text font-medium">{point.label}:</span>{" "}
      <span className="text-text-secondary">{parseEmphasis(point.text)}</span>
    </>
  );
}
    ...
    >
      {hasContent &&
        concept.explanation.map((item, i) => (
          ...
          >
            {typeof item === "string" ? (
              <span className="text-text-secondary">{parseEmphasis(item)}</span>
            ) : (
